  ...
  return (
    ...
    >
      <div className="space-y-3">
        {/* Medication task — always present */}
        <Link to="/patient/medications" className="flex items-start gap-3 group min-h-[44px]">
          ...
          <div className="flex-1">
            <p
              className={`text-sm font-body font-semibold ${allMedsTaken ? 'text-muted-foreground line-through' : 'text-foreground'}`}
            >
              Take medication
            </p>
            <p className="text-xs text-muted-foreground font-body">
              {isPast
                ? allMedsTaken
                  ? 'All doses taken'
                  : `${medsTakenCount} of ${medsTotal} taken`
                : isCurrentDay
        ...
        {appointments.map((appt) => {
          ...
          return (
            <div key={appt.id} className="flex items-start gap-3 min-h-[44px]">
              ...
              <div className="flex-1">
                <p
                  className={`text-sm font-body font-semibold ${done ? 'text-muted-foreground line-through' : 'text-foreground'}`}
                >
                  Follow-up{appt.doctor_name ? ` with ${appt.doctor_name}` : ''} at {time}
                </p>
                {appt.reason && (
                  <p className="text-xs text-muted-foreground font-body">{appt.reason}</p>
                )}
              </div>
        ...
        {isCheckUpDay && (
          ...
          >
            ...
            <div className="flex-1">
              <p
                className={`text-sm font-body font-semibold ${checkUpDone ? 'text-muted-foreground line-through' : 'text-foreground'}`}
              >
