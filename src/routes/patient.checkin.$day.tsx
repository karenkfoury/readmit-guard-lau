...
function CheckInSurvey() {
  ...
  const handleSubmit = () => {
    const dayNum = parseInt(day) as 3 | 7 | 14;
    const updatedCheckIns = patient.checkIns.map(c =>
      c.day === dayNum ? { ...c, status: 'completed' as const, completedAt: new Date().toISOString(), responses: answers } : c
    );
    updatePatient('p1', { checkIns: updatedCheckIns });
    setSubmitted(true);
  };
    ...
    return (
      <div className="min-h-screen bg-lau-bg flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-sm">
          ...
          <p className="text-sm text-muted-foreground font-body mb-8 bg-primary/5 rounded-lg p-3 border border-primary/20">
            🩺 Your care team may contact you for a follow-up if needed.
          </p>
          <button onClick={() => navigate({ to: '/patient' })} className="bg-primary text-primary-foreground px-6 py-3 rounded-xl font-heading font-semibold hover:bg-lau-green-dark transition-colors">
            Back to Dashboard
          </button>
        </motion.div>
      </div>
          ...
          <motion.div key={step} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-lg">
            ...
            {current.type === 'number' && (
              <input type="number" value={answers[current.id] || ''} onChange={e => setAnswer(Number(e.target.value))}
                className="w-full text-center text-3xl font-heading font-bold py-4 border-2 border-lau-border rounded-xl bg-card focus:outline-none focus:border-primary"
                placeholder="0"
              />
            )}
            ...
            {current.type === 'text' && (
              <textarea value={answers[current.id] || ''} onChange={e => setAnswer(e.target.value)}
                className="w-full p-4 border-2 border-lau-border rounded-xl bg-card font-body focus:outline-none focus:border-primary resize-none"
                rows={4} placeholder="Type here (optional)..."
              />
            )}
          </motion.div>
        </AnimatePresence>
      ...
      <div className="p-4 bg-card border-t border-lau-border">
        <div className="max-w-lg mx-auto">
          <button onClick={() => isLast ? handleSubmit() : setStep(step + 1)}
            className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-heading font-semibold text-lg hover:bg-lau-green-dark transition-colors flex items-center justify-center gap-2"
          >
            {isLast ? 'Submit Check-In' : 'Next'} {!isLast && <ArrowRight className="h-5 w-5" />}
          </button>
        </div>
      </div>
    </div>
