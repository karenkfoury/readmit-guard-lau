...
export const Route = createFileRoute('/')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'AI-powered post-discharge monitoring to prevent 30-day hospital readmissions. Built by LAU Health.' },
    ],
  }),
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-primary text-primary-foreground">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-primary-foreground/70 text-xs font-body ml-1 hidden sm:inline">| LAU Health</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="bg-primary-foreground/20 text-primary-foreground text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider">Demo Mode</span>
            <Link to="/login" className="text-sm font-body hover:underline">Sign In</Link>
          </div>
        </div>
      </header>
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-heading text-3xl font-bold text-center text-foreground mb-12">The Real Cost of Readmissions</h2>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: <Heart className="h-8 w-8" />, title: 'For Patients', desc: 'Extended hospital stays, higher mortality risk, emotional toll, and disrupted recovery.' },
              { icon: <DollarSign className="h-8 w-8" />, title: 'For Hospitals', desc: '$26 billion annually in the US. CMS penalties reduce Medicare payments for high readmission rates.' },
              { icon: <Hospital className="h-8 w-8" />, title: 'For Healthcare', desc: 'Overwhelmed ERs, wasted resources, and preventable suffering that erodes public trust.' },
            ].map((card, i) => (
              <motion.div
                key={card.title}
              ...
              >
                <div className="h-14 w-14 rounded-xl bg-primary/10 flex items-center justify-center text-primary mb-4">{card.icon}</div>
        ...
        <div className="max-w-4xl mx-auto px-4">
          ...
          >
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h3 className="font-heading text-2xl font-bold text-foreground">Meet Sarah</h3>
            ...
            >
              See how we would have caught this <ArrowRight className="h-4 w-4" />
            </Link>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="font-heading text-3xl font-bold text-center text-foreground mb-12">Choose Your View</h2>
          <div className="grid md:grid-cols-2 gap-8">
            <Link to="/login" onClick={() => setRole('patient')}>
              <motion.div whileHover={{ scale: 1.02 }} className="rounded-2xl border-2 border-lau-border bg-card p-8 text-center hover:border-primary transition-all cursor-pointer shadow-sm hover:shadow-lg group">
                ...
                <h3 className="font-heading text-xl font-bold text-foreground mb-2">Continue as Patient</h3>
            ...
            <Link to="/login" onClick={() => setRole('doctor')}>
              <motion.div whileHover={{ scale: 1.02 }} className="rounded-2xl border-2 border-lau-border bg-card p-8 text-center hover:border-primary transition-all cursor-pointer shadow-sm hover:shadow-lg group">
                ...
                <h3 className="font-heading text-xl font-bold text-foreground mb-2">Continue as Doctor</h3>
      ...
      <footer className="bg-primary text-primary-foreground py-8">
        <div className="max-w-6xl mx-auto px-4 text-center">
          <p className="font-body text-sm text-primary-foreground/70">
            ReAdmit Guard is a prototype developed as part of a Lebanese American University Health initiative.
          </p>
          <p className="text-xs text-primary-foreground/50 mt-2 font-body">© 2026 LAU Health — For demonstration purposes only</p>
        </div>
      </footer>
    </div>
  );
