    ...
    <div className="min-h-screen bg-lau-bg">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <KPICard label="Patients Monitored" value={patients.length} icon={<Users className="h-6 w-6" />} />
          <KPICard label="High Risk (>50%)" value={highRisk.length} icon={<AlertTriangle className="h-6 w-6" />} />
          <KPICard label="Follow-Ups Today" value={pendingFollowUps.length} icon={<Clock className="h-6 w-6" />} />
          <KPICard label="Check-Ins This Week" value={checkInsThisWeek} icon={<CheckCircle className="h-6 w-6" />} />
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="flex gap-1 flex-wrap">
                {(['all', 'high', 'moderate', 'low', 'followup'] as const).map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-heading font-semibold transition-colors ${filter === f ? 'bg-primary text-primary-foreground' : 'bg-card border border-lau-border text-foreground hover:bg-accent'}`}
                  >
              ...
              <div className="flex items-center gap-2 ml-auto">
                <div className="relative">
                  ...
                  <input id="doc-search" value={search} onChange={e => setSearch(e.target.value)}
                    ...
                    className="pl-9 pr-3 py-1.5 rounded-lg border border-lau-border bg-card text-sm font-body focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary w-48"
                  />
                </div>
                <div className="flex border border-lau-border rounded-lg overflow-hidden">
                  <button onClick={() => setView('grid')} className={`p-1.5 ${view === 'grid' ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground'}`}>
                    <LayoutGrid className="h-4 w-4" />
                  </button>
                  <button onClick={() => setView('list')} className={`p-1.5 ${view === 'list' ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground'}`}>
          ...
          <div className="w-full lg:w-80 space-y-4">
            {/* Action Queue */}
            <div className="rounded-xl border border-lau-border bg-card p-5 shadow-sm">
              <h3 className="font-heading font-semibold text-foreground mb-4">Today's Action Queue</h3>
              <div className="space-y-3">
                {followUps.filter(f => f.status === 'today').map(f => (
                  <div key={f.id} className="p-3 rounded-lg bg-lau-bg border border-lau-border">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-heading font-semibold text-foreground">{f.patientName}</span>
                      <RiskBadge score={f.riskScore} size="sm" />
                    </div>
                    <p className="text-xs text-muted-foreground font-body mb-2">{f.reason}</p>
                    <div className="flex gap-2">
                      <button className="flex-1 text-xs py-1 rounded bg-primary text-primary-foreground font-semibold hover:bg-lau-green-dark transition-colors">Complete</button>
                      <button className="flex-1 text-xs py-1 rounded border border-lau-border text-foreground font-semibold hover:bg-accent transition-colors">Reassign</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 bg-primary text-primary-foreground px-5 py-3 rounded-full shadow-lg hover:bg-lau-green-dark transition-colors font-heading font-semibold flex items-center gap-2 z-50"
        onClick={() => {
